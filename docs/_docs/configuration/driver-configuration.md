...
```php
\Twigger\Translate\Setting::driver('my-configuration')->translate('Text to Setting', 'en', 'fr');
```
...
```php
\Twigger\Translate\Setting::pushConfiguration('my-configuration', 'driver-name', [
    'username' => 'Some Username',
    'password' => 'pw'
]);
```
...
```php
\Twigger\Translate\Setting::setDefaultDriver('my-configuration');
```
